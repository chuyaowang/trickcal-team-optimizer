import time
import argparse
import json
import os
from src.data_loader.csv_loader import load_pets, load_tasks, get_available_job_files
from src.core.scoring import precompute_pet_task_scores, get_reward_level
from src.core.assignment import calculate_best_assignment
from src.core.constants import SKILL_SCORE_MAP
from src.core.i18n import t
from src.ui.cli import (
    select_server, select_job_file, show_pets, 
    select_owned_pets, select_farm_pets, select_task_count, 
    display_results
)

def run_config(config_path, lang='cn'):
    """根据配置文件直接运行计算"""
    if not os.path.exists(config_path):
        print(f"Error: Config file {config_path} not found.")
        return

    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
    except Exception as e:
        print(f"Error: Failed to read config. {e}")
        return

    server = config.get('server', 'cn')
    max_active_jobs = config.get('max_job_number', 5)
    owned_pets_list = config.get('owned_pets', [])
    aux_pets_counts = config.get('aux_pets_counts', {})
    ui_lang = config.get('ui_language', lang)

    # 加载数据
    try:
        all_pets = load_pets(server=server)
    except Exception as e:
        print(f"Error: {e}")
        return

    job_files = get_available_job_files(server=server)
    if not job_files:
        print(f"Error: No tasks in data/{server}")
        return
    selected_job_file = job_files[-1] 
    tasks = load_tasks(selected_job_file)

    my_pets = [p for p in all_pets if p['name'] in owned_pets_list]
    
    print(f"\n{t('STATUS', ui_lang)}: {t('RUN_OPTIMIZER', ui_lang)}")
    print(f"Server: {server.upper()} | Language: {ui_lang}")
    
    start_time = time.time()
    pet_task_scores = precompute_pet_task_scores(all_pets, tasks)
    result = calculate_best_assignment(
        my_pets=my_pets,
        aux_pets_counts=aux_pets_counts,
        tasks=tasks,
        pet_task_scores=pet_task_scores,
        max_active_jobs=max_active_jobs
    )
    calc_time = time.time() - start_time
    display_results(result, server, ui_lang, calc_time)

def main():
    parser = argparse.ArgumentParser(description="Pet Dispatch Optimizer (MILP)")
    parser.add_argument("--config", type=str, help="Path to config (.json)")
    parser.add_argument("--lang", type=str, choices=['cn', 'en'], help="UI Language")
    args = parser.parse_args()

    # Determine language
    lang = args.lang
    if not lang:
        print("1. 简体中文")
        print("2. English")
        choice = input("Select Language (default 1): ")
        lang = 'en' if choice == '2' else 'cn'

    if args.config:
        run_config(args.config, lang)
        return

    # --- Interactive CLI ---
    print(f"\n=== {t('APP_TITLE', lang)} ===")
    
    server = select_server(lang)
    
    try:
        all_pets = load_pets(server=server)
    except Exception as e:
        print(f"Error: {e}")
        return

    job_files = get_available_job_files(server=server)
    if not job_files:
        print(f"Error: data/{server} is empty")
        return
    
    selected_job_file = select_job_file(job_files, lang)
    tasks = load_tasks(selected_job_file)

    show_pets(all_pets, SKILL_SCORE_MAP, lang)
    my_pets = select_owned_pets(all_pets, lang)
    aux_pets_counts = select_farm_pets(all_pets, lang)
    max_active_jobs = select_task_count(lang)

    print(f"\n{t('PROGRESS', lang)}...")
    start_time = time.time()
    pet_task_scores = precompute_pet_task_scores(all_pets, tasks)
    result = calculate_best_assignment(
        my_pets=my_pets,
        aux_pets_counts=aux_pets_counts,
        tasks=tasks,
        pet_task_scores=pet_task_scores,
        max_active_jobs=max_active_jobs
    )
    calc_time = time.time() - start_time

    display_results(result, server, lang, calc_time)

if __name__ == "__main__":
    main()
